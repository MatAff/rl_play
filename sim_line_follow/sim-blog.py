...
# Control class based on an engineered approach
class RLEngineered(RLLineFollow):

	def __init__(self, state_space):
		super(RLEngineered, self).__init__(state_space)

	def pre(self, run_nr):
		super(RLEngineered, self).pre(run_nr)

	def post(self, run_nr):
		super(RLEngineered, self).post(run_nr)

	def decide(self, line_pos):
		super(RLEngineered, self).decide(line_pos)
...
# RI control class based on mimicing the engineered approach
class RLMimic(RLLineFollow):

	def __init__(self, state_space):
		super(RLMimic, self).__init__(state_space)

	def pre(self, run_nr):
		super(RLMimic, self).pre(run_nr)
	...
	def post(self, run_nr):
		super(RLMimic, self).post(run_nr)

	def decide(self, line_pos):
		super(RLMimic, self).decide(line_pos)
		if self.phase == 0:
			if self.line_pos[1] is not None:
...
class RLSubData(RLLineFollow):

	def __init__(self, state_space):
		super(RLSubData, self).__init__(state_space)

	def pre(self, run_nr):
		super(RLSubData, self).pre(run_nr)
	...
	def post(self, run_nr):
		super(RLSubData, self).post(run_nr)

	def decide(self, line_pos):
		super(RLSubData, self).decide(line_pos)
		if self.phase == 0:
			if self.line_pos[1] is not None:
				rotate = self.line_pos[1] * 0.1
		else:
...
#control = RLEngineered(len(dist_list))
#control = RLMimic(len(dist_list))
#control = RLSubData(len(dist_list))
control = RLStateAction(len(dist_list))
